...
from .services import cleanup_stored_emails, decrypt_token, encrypt_token, _looks_like_schedule_email
...
class ScheduleHeuristicTests(SimpleTestCase):
    def test_japanese_schedule_signals_are_detected(self):
        class Message:
            subject = '面接日程のご案内'
            body_text = '候補日をご確認ください。'

        self.assertTrue(_looks_like_schedule_email(Message()))

    def test_unrelated_newsletter_is_not_selected(self):
        class Message:
            subject = '今週のおすすめ記事'
            body_text = '新しい記事をお届けします。'

        self.assertFalse(_looks_like_schedule_email(Message()))

    def test_terse_date_and_time_are_detected_without_schedule_keyword(self):
        class Message:
            subject = 'ご案内'
            body_text = '2026年8月20日 15:30 にお越しください。'

        self.assertTrue(_looks_like_schedule_email(Message()))

    def test_resume_submission_and_aptitude_action_signals_are_detected(self):
        class ResumeMessage:
            subject = '履歴書提出のお願い'
            body_text = '応募ページからアップロードしてください。'

        class AssessmentMessage:
            subject = 'Action required'
            body_text = 'Please complete the aptitude test by Friday.'

        self.assertTrue(_looks_like_schedule_email(ResumeMessage()))
        self.assertTrue(_looks_like_schedule_email(AssessmentMessage()))
