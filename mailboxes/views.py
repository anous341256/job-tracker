...
from core.models import HostAgentCommand, HostAgentHeartbeat
...
def accounts(request):
    return render(
        request,
        'mailboxes/accounts.html',
        {
            'accounts': EmailAccount.objects.filter(user=request.user),
            'local_outlook_available': sys.platform == 'win32' or settings.HOST_AGENT_ENABLED,
            'host_agent': HostAgentHeartbeat.objects.order_by('-last_seen_at').first(),
        },
    )
...
def connect_local_outlook(request):
    ...
    if settings.HOST_AGENT_ENABLED:
        HostAgentCommand.objects.get_or_create(
            user=request.user, command_type=HostAgentCommand.Type.OUTLOOK_CONNECT,
            status__in=(HostAgentCommand.Status.QUEUED, HostAgentCommand.Status.RUNNING),
        )
        messages.info(request, '已请求 Windows 本机代理连接 Outlook，请稍后刷新。')
        return redirect('mailboxes:accounts')
    if sys.platform != 'win32':
        messages.error(request, '本机 Outlook 同步只支持 Windows 宿主机，Docker/Linux 容器中不可用。')
        return redirect('mailboxes:accounts')
    try:
...
def message_detail(request, pk):
    ...
    return render(request, 'mailboxes/message_detail.html', {'email': email, 'attachments': attachments, 'attachment_error': attachment_error, 'ai_settings': AISettings.objects.filter(user=request.user).first()})


@login_required
def analyze_schedule(request, pk):
    email = get_object_or_404(SyncedEmail, pk=pk, account__user=request.user)
    if request.method == 'POST':
        try:
            task, created = create_email_schedule_task(user=request.user, email=email, provider=request.POST.get('provider') or None)
            messages.success(request, '已提交邮件日程分析。' if created else '该邮件已有分析任务正在运行。')
...
@login_required
def batch_analyze_schedule(request):
    if request.method == 'POST':
        emails = SyncedEmail.objects.filter(account__user=request.user, pk__in=request.POST.getlist('selected'))[:20]
        queued = 0
        for email in emails:
            try:
                _, created = create_email_schedule_task(user=request.user, email=email, provider=request.POST.get('provider') or None)
                queued += int(created)
            except ValueError as exc:
                messages.error(request, str(exc))
                break
        if queued:
            messages.success(request, f'已提交 {queued} 封邮件的日程分析。')
...
def sync(request, pk):
    account = get_object_or_404(EmailAccount, pk=pk, user=request.user)
    if request.method == 'POST':
        if account.provider == EmailAccount.Provider.OUTLOOK_LOCAL and settings.HOST_AGENT_ENABLED:
            command, created = HostAgentCommand.objects.get_or_create(
                user=request.user, email_account=account, command_type=HostAgentCommand.Type.OUTLOOK_SYNC,
                status__in=(HostAgentCommand.Status.QUEUED, HostAgentCommand.Status.RUNNING),
                defaults={'payload': {'limit': 50, 'days': 30, 'folder': account.sync_folder}},
            )
            messages.info(request, '已提交 Outlook 同步请求，Windows 本机代理完成后会自动保存邮件。' if created else '已有 Outlook 同步任务正在等待本机代理。')
            return redirect('mailboxes:accounts')
        if account.provider == EmailAccount.Provider.OUTLOOK_LOCAL and sys.platform != 'win32':
            messages.error(request, '本机 Outlook 同步只支持 Windows 宿主机，Docker/Linux 容器中不可用。')
            return redirect('mailboxes:accounts')
        try:
